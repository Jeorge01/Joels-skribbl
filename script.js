...
const manageCanvas = setupCanvas(
  ...
  (event, ctx) => {
    const newCoords = drawingUtil.draw(
      event,
      isDrawing,
      lastX,
      lastY,
  ...
  () => {
    const result = drawingUtil.stopDrawing(
      isDrawing,
      currentStroke,
      strokeHistory
    );
...
document.addEventListener("keydown", (e) => {
  ...
  if (
    e.code === "Space" &&
    !isSpacePressed &&
    document.activeElement !== document.querySelector("#chatInput")
  ) {
    e.preventDefault();
    isSpacePressed = true;
    isDrawing = true;
    ...
    .map(
      (player) =>
        `<li data-player-id="${player.id}" ${
          ...
          !player.painter && player.knowsWord ? "✓" : ""
        }</span> <span>${
...
function createWinnerElement(data) {
  ...
  const winnerName =
    data.podium.find((player) => player.position === 1)?.name || "No winner??";
