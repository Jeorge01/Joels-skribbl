...
function loadWords() {
    try {
        ...
        if (!Array.isArray(words)) {
            throw new Error("Words data is not an array. Check words.json structure!");
        }
...
function startGame(ws) {
    ...
    const painterWs = Array.from(clients.entries()).find(([socket, client]) => client.id === firstPlayer.id)?.[0];
...
function rotateTurn() {
    ...
    const wordChoices = chooseWords();
    const painter = Array.from(clients.entries()).find(([_, client]) => client.id === currentPlayer.id);
...
function broadcast(message, sender) {
    ...
    if (parsedMessage.type === "chat" && parsedMessage.message === currentWord && senderData.painter) {
        return;
    }
    ...
    if (parsedMessage.type === "chat" && parsedMessage.message === currentWord) {
        ...
        const allPlayersKnowWord = Array.from(clients.values()).every((player) => player.knowsWord || player.painter);
